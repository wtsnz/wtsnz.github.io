...
I've been wanting to compare a api implementation in two different ways. We have an existing API that is written in node and express that runs on AWS Lambda. I'd like to provide an alternate implemenation that uses Elixir and Phoenix so I can compare the two approaches.
...
[Terraform](https://github.com/poteto/terraform) is a library that allows you to do just this. I can setup an Elixir API that proxies requests to the existing API, and provide implemenations for each method of the existing API as I go.
