...
<!-- at the end of the Locking Script, using the OP_RETURN opcode as a 'separator' of sorts and then access the data inside the Unlocking Script. We can also access the current transaction. This allows us to make any sort of arbitrary restrictions on the transaction that you can imagine. It's trivial to require the output amounts to be a certain value. It's trivial to require that the code part of the locking script hasn't changed so that the contract must continue so that it lives forever. -->
