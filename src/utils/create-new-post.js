...
const getParam = arg => (process.argv.find(param => param.split('=')[0] === arg) || '').split('=')[1]

//long or short ISO timezones
const niceNow = (short = true, now = new Date()) => short ? now.toISOString().split('T')[0] : now.toISOString()
...
const readBlogTemplate = blog => {
    ...
    const promise = new Promise((resolve, reject) => {
        fs.readFile(
            path.join(__dirname, './emptyBlogPostTemplate.md'),
            'utf8',
            (err, data) => {
                if (err) reject(err)
                const template = data
